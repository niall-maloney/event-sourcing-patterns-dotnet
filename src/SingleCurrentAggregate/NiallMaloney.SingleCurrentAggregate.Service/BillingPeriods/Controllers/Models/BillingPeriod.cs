...
public record BillingPeriod(string? Id, string CustomerId, string? Status, decimal TotalAmount, ulong Version)
{
    ...
    public static IEnumerable<BillingPeriod> Map(IEnumerable<BillingPeriodRow> rs) =>
        rs.Select(r => new BillingPeriod(r.BillingPeriodId, r.CustomerId, r.Status, r.TotalAmount, r.Version));
}